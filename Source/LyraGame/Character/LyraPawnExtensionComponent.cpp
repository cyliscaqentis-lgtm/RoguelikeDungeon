...
{
	if (!InASC || !InOwnerActor)
	{
		UE_LOG(LogLyra, Warning, TEXT("InitializeAbilitySystem: invalid args ASC=%p Owner=%p"), InASC, InOwnerActor);
		return;
	}

	if (AbilitySystemComponent == InASC)
	{
		// Same ASC - skip (無害なので VeryVerbose に降格)
		UE_LOG(LogLyra, VeryVerbose, TEXT("InitializeAbilitySystem: same ASC, skip (%s)"), *GetNameSafe(InOwnerActor));
		return;
	}

	if (AbilitySystemComponent && AbilitySystemComponent != InASC)
	{
		// 別ASCで再初期化 - 実害はないが Warning で通知してスキップ
		UE_LOG(LogLyra, Warning, TEXT("InitializeAbilitySystem: already initialized with another ASC, skip (%s)"), *GetNameSafe(InOwnerActor));
		return;
	}
