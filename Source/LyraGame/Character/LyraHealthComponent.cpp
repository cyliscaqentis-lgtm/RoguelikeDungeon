...
void ULyraHealthComponent::InitializeWithAbilitySystem(ULyraAbilitySystemComponent* InASC)
{
	...
	if (!InASC)
	{
		UE_LOG(LogLyra, Warning, TEXT("LyraHealthComponent: InitializeWithAbilitySystem ASC=null, skip"));
		return;
	}

	if (AbilitySystemComponent == InASC)
	{
		UE_LOG(LogLyra, VeryVerbose, TEXT("LyraHealthComponent: already initialized with same ASC, skip"));
		return;
	}

	if (AbilitySystemComponent && AbilitySystemComponent != InASC)
	{
		UE_LOG(LogLyra, Warning, TEXT("LyraHealthComponent: reinit with different ASC requested, skip"));
		return;
	}
